...
#### 3.1.5 Quanto-Inverse 扩展

代码：
- `research/pricing/quanto_inverse.py`
- `research/hedging/quanto_inverse.py`

核心路径：
- 先计算 inverse 基础价格与 Greeks；
- 叠加 quanto 调整项 `exp(-rho * sigma_spot * sigma_fx * T)`；
- 按 `fx_rate` 换算到结算币种。

风险暴露新增：
- `fx_delta`：FX 敏感度
- `corr_sensitivity`：相关性敏感度
- `QuantoInverseHedger.build_hedge_plan(...)`：spot + FX 双腿对冲建议
...
- 跨市场 live 偏离：`/api/deviation/live`（CEX + DeFi 对齐后输出偏离热力图与告警）
...
2. `research/pricing/model_zoo.py`
3. `research/pricing/quanto_inverse.py`
4. `research/signals/jump_risk_premia.py`
5. `research/signals/bayesian_changepoint.py`
6. `research/hedging/deep_hedging.py`
7. `research/execution/almgren_chriss.py`
8. `data/quote_integration.py`
