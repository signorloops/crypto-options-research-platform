...
- `Makefile` 的 `quality` / `test-cov` / `weekly-close-gate`
...
注入方式：
1. 使用 `scripts/performance/stress_test.py` 的闪崩场景。
2. 或回放历史极端行情样本。
...
使用：
- `scripts/performance/latency_benchmark.py`
...
历史上存在 `verify_all.sh`（已移除），当前建议使用：

- `make quality`
- `make test-cov`
- `make weekly-close-gate`

建议把上述链路纳入发布流水线，在 staging 作为硬门禁。
...
标定与性能：
1. `scripts/performance/latency_benchmark.py`
2. `scripts/performance/stress_test.py`
3. `make quality && make test-cov && make weekly-close-gate`
