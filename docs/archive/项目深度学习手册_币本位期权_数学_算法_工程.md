...
./venv/bin/python validation_scripts/inverse_power_validation.py
```
历史 one-off 验证脚本已下线，保留报告见 `docs/archive/reports/`。
...
3. `scripts/performance/latency_benchmark.py`
...
# 3) 性能回归子集
./venv/bin/python scripts/performance/latency_benchmark.py \
  --iterations 150 \
