...
- 手动触发时可调 `seed`、`n_per_bucket`、`quotes_json`、`expected_best_model`、`max_best_rmse`、`max_best_rmse_increase_pct`、`max_iv_reduction_drop_pct`、`allow_best_model_change`、`fail_on_arbitrage`、`min_short_max_jump_reduction`、`min_net_jump_premium_std`、`max_inverse_power_abs_error`。
