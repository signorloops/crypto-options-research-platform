...
class TestPortfolioGreeks:
    ...
    def test_initialization(self):
        ...
        pg = PortfolioGreeks(
            delta=0.5, gamma=0.01, theta=-0.1, vega=0.2, rho=0.05, vanna=0.01, charm=-0.01, veta=0.0
        )
        assert pg.delta == 0.5
...
class TestGreeksRiskAnalyzer:
    ...
    def test_analyze_position(self):
        ...
        per_contract, position_greeks = analyzer.analyze_position(
            position=position, contract=contract, spot=100.0, implied_vol=0.5, as_of=datetime.now()
        )
    ...
    def test_find_hedge_ratio(self):
        ...
        portfolio_greeks = PortfolioGreeks(
            delta=5.0, gamma=0.1, theta=-0.5, vega=1.0, rho=0.1, vanna=0.01, charm=-0.01, veta=0.0
        )
    ...
    def test_calculate_greeks_scenarios(self):
        ...
        scenarios = analyzer.calculate_greeks_scenarios(
            positions, as_of, spot_shocks=[0.95, 1.0, 1.05], vol_shocks=[-0.1, 0, 0.1]
        )
...
class TestVaRCalculator:
    ...
    def test_parametric_var(self):
        ...
        positions = pd.DataFrame({"value": [50000, 50000]}, index=["BTC", "ETH"])

        # Create returns DataFrame
        np.random.seed(42)
        returns = pd.DataFrame(
            {"BTC": np.random.normal(0, 0.02, 1000), "ETH": np.random.normal(0, 0.025, 1000)}
        )
    ...
    def test_monte_carlo_var_with_seed_is_reproducible(self):
        ...
        r1 = calc.monte_carlo_var(
            positions,
            returns,
            n_simulations=3000,
            random_seed=12345,
        )
        r2 = calc.monte_carlo_var(
            positions,
            returns,
            n_simulations=3000,
            random_seed=12345,
    ...
    def test_var_result_to_dict(self):
        ...
        result = VaRResult(
            var_95=1000.0, var_99=2000.0, cvar_95=1500.0, cvar_99=2500.0, method="parametric"
        )
        d = result.to_dict()
...
class TestStressTest:
    ...
    def test_run_stress_test(self):
        ...
        positions = pd.DataFrame({"value": [100000, 50000]}, index=["BTC-CALL", "BTC-PUT"])

        greeks = pd.DataFrame(
            {"delta": [0.5, -0.3], "gamma": [0.01, 0.01], "vega": [0.2, 0.15]},
    ...
    def test_custom_scenario(self):
        ...
        positions = pd.DataFrame({"value": [100000]}, index=["BTC-CALL"])

        greeks = pd.DataFrame({"delta": [0.5], "gamma": [0.01], "vega": [0.2]}, index=["BTC-CALL"])
