...
class BlackScholesGreeks:
    ...
    def calculate(
        ...
        if S <= 0: raise ValueError(f"Spot price S must be positive, got {S}")
        if K <= 0: raise ValueError(f"Strike price K must be positive, got {K}")
        if sigma <= 0: raise ValueError(f"Volatility sigma must be positive, got {sigma}")
        if T <= 0: return Greeks(delta=0, gamma=0, theta=0, vega=0, rho=0)
...
class GreeksRiskAnalyzer:
    ...
    def _contract_greeks(
        ...
        *,
        ...
        return self.calculator.calculate(S=spot, K=contract.strike, T=contract.time_to_expiry(as_of), r=self.risk_free_rate, sigma=implied_vol, option_type=option_type)
    ...
    def analyze_portfolio(
        ...
        """Aggregate portfolio Greeks by underlying currency, optionally FX-converted."""
    ...
    def calculate_greeks_scenarios(
        ...
        """Calculate scenario PnL grid under spot and volatility shocks."""
        spot_shocks = spot_shocks or self._default_spot_shocks(); vol_shocks = vol_shocks or self._default_vol_shocks()
        results = []
        for spot_mult in spot_shocks:
            for vol_change in vol_shocks:
                ...
                results.append({"spot_shock": f"{spot_mult:.0%}", "vol_shock": f"{vol_change:+.0%}", "pnl": pnl})
