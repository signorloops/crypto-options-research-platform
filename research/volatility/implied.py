...
class VolatilitySurface:
    ...
    def get_volatility(self, strike: float, expiry: float,
        ...
        if method == "ssvi":
            ...
            if self._ssvi_params is not None and self._ssvi_atm_expiries is not None and self._ssvi_atm_total_vars is not None:
                theta = np.interp(
                    float(expiry),
    ...
    def get_skew(self, expiry: float) -> List[Tuple[float, float]]:
        """
        ...
        return [(p.moneyness, p.volatility) for p in sorted(points, key=lambda x: x.strike)]
